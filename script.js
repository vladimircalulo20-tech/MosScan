...
try {
  ...
  console.error("Could not load saved records:", error);
  records = [];
}
...
// Candijay, Bohol approximate center
const CANDIJAY_LATITUDE = 9.8172;
const CANDIJAY_LONGITUDE = 124.4960;

// Create the map
const map = L.map("map").setView(
  [CANDIJAY_LATITUDE, CANDIJAY_LONGITUDE],
  13
);
...
async function loadAIModel() {
  try {
    ...
    console.log("MosquiScan AI model loaded successfully.");
    ...
    confidenceText.textContent =
      "Check your internet connection or model link.";
  }
}
...
analyzeButton.addEventListener("click", async function () {
  ...
  try {
    ...
    console.log("All AI predictions:", predictions);
...
function displayRecordsOnMap() {
  ...
  records.forEach(function (record) {
    const possibleSite = isPossibleBreedingSite(
      record.result
    );
...
saveButton.addEventListener("click", function () {
  ...
  if (
    ...
    result === "AI model failed to load" ||
    result === ""
  ) {
...
function displayActivityLog() {
  ...
  newestRecords.forEach(function (record) {
    ...
    logEntry.innerHTML = `
      ...
      <span>${record.confidence}</span><br>
      <span>
